...
describe('resolve_target', function()
  ...
  it('strips oil:// scheme from buffer name', function()
    local buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_set_current_buf(buf)
    vim.api.nvim_buf_set_name(buf, 'oil:///home/user/projects/')
    assert.are.equal('/home/user/projects/', resolve_target(nil))
    vim.api.nvim_buf_delete(buf, { force = true })
  end)

  it('resolves diffview:// buffer to real file path', function()
    local buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_set_current_buf(buf)
    vim.api.nvim_buf_set_name(buf, 'diffview:///Users/tai/project/.git/:0:/lua/init.lua')
    assert.are.equal('/Users/tai/project/lua/init.lua', resolve_target(nil))
    vim.api.nvim_buf_delete(buf, { force = true })
  end)

  it('resolves gitsigns:// buffer to real file path', function()
    local buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_set_current_buf(buf)
    vim.api.nvim_buf_set_name(buf, 'gitsigns:///Users/tai/project/.git//:0:lua/init.lua')
    assert.are.equal('/Users/tai/project/lua/init.lua', resolve_target(nil))
    vim.api.nvim_buf_delete(buf, { force = true })
  end)

  it('resolves fugitive:// stage buffer to real file path', function()
    local buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_set_current_buf(buf)
    vim.api.nvim_buf_set_name(buf, 'fugitive:///Users/tai/project/.git//0/lua/init.lua')
    assert.are.equal('/Users/tai/project/lua/init.lua', resolve_target(nil))
    vim.api.nvim_buf_delete(buf, { force = true })
  end)

  it('resolves fugitive:// commit buffer to real file path', function()
    local buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_set_current_buf(buf)
    vim.api.nvim_buf_set_name(buf, 'fugitive:///Users/tai/project/.git//abc123def0abc123def0abc123def0abc123def0/src/main.rs')
    assert.are.equal('/Users/tai/project/src/main.rs', resolve_target(nil))
    vim.api.nvim_buf_delete(buf, { force = true })
  end)
end)
