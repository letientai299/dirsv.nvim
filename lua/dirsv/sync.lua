...
--- Returns one or two payloads: scroll is always sent so the browser
--- tracks the viewport; cursor or selection is sent on top.
...
local function build_payloads()
  ...
  -- Visual mode: send scroll + selection. \22 = CTRL-V (visual block).
  ...
  -- Normal/insert mode: send scroll + cursor.
  local cursor = vim.api.nvim_win_get_cursor(0)
  local line = cursor[1]
  local cur = vim.json.encode({
    type = "cursor",
    path = rel,
    line = line,
  })
...
M._test = {
  ...
  http_post = http_post,
  get_state = function() return st end,
}
